...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredItemAccordingInput = filteredItem.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(inputText.toLowerCase()),
    )
    return (
      <div className="main-container">
        ...
        <div>
          <input
            onChange={this.inputChangingItem}
            type="search"
            placeholder="Search"
          />
          <img
        ...
        <br />
        <ul className="projectItem-container">
          {filteredItemAccordingInput.map(eachItem => (
            <AppItem key={eachItem.id} projectDetails={eachItem} />
          ))}
        </ul>
      </div>
    )
